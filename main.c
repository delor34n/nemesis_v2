...
int carga_texturas() {
    ...
    for(i = 0; i < NUM_TEXTURAS; i++)
    {
        ...
        gluBuild2DMipmaps (GL_TEXTURE_2D, 4, tam[i], tam[i], GL_RGBA,
                        GL_UNSIGNED_BYTE, textura_datos[i]);
...
//glutSolidShare(10,slices,stacks);
...
int RenderVertices(int renderPrimitive, /*BYTE*/char  *pVertex, unsigned int vertexSize, unsigned int vStart, unsigned int numVertex) //,unsigned int vertexFormat )
{
	...
	glVertexPointer( 3, GL_FLOAT	,vertexSize, pVertex);	//(3,El vértice se compone de 3 nº reales., Tamaño de la estructura que define el vértice., Dónde empieza el primer vrtice a renderizar.)
	offset += 3*sizeof(float);
	// Especificamos las normales
	glEnableClientState	(GL_NORMAL_ARRAY);
	glNormalPointer	( GL_FLOAT, vertexSize, pVertex+offset);	//( Tipo de coordenada de la normal., Tamaño de la estructura que define el vértice.,Dónde empieza la primera normal.)
	offset += 3*sizeof(float);
	// Especificamos las coordenadas de textura
	glEnableClientState	( GL_TEXTURE_COORD_ARRAY );
	glTexCoordPointer	( 2, GL_FLOAT, vertexSize, pVertex+offset );		//(Nº de coordenadas de textura.,Tipo de coordenadas de textura.,Tamaño de la estructura que define el vértice.,Dónde empieza la primera coordenada.)
...
void render_esfera(float m_Radius, int sl, int st , int id_textura)
{
    ...
    //RenderVertices( 4, (BYTE*)m_Vertex, sizeof(vertice), 0, numVertex);//, eFVF_XYZ|eFVF_NORMAL|eFVF_TEX1 );
    RenderVertices( 4, (char *)m_Vertex, sizeof(vertice), 0, numVertex);//, eFVF_XYZ|eFVF_NORMAL|eFVF_TEX1 );
    }
    glDisable(GL_TEXTURE_2D);
}

//funcion creadora de estrellas (NO USADA)
...
int sw=10;
//Funcion creadora de meteoritos
void meteoro (float x, float y, float z){
	...
	/*glMaterialfv (GL_FRONT, GL_SPECULAR, mat_specular_pluton);
	glMaterialfv (GL_FRONT, GL_AMBIENT, mat_ambient_pluton);
	glLightf(GL_LIGHT1, GL_SPOT_CUTOFF, 360.0);*/
   	anos = ((float) dia / 365)*360;
	 //  glRotatef( anos/320, 0,1,0 );
/*	float x=-13.4;
	float z=-7.0;*/
	/*
	float z3=0;
	...
	while(i<18){
	if(a[i]==x)
	if(sqrt((x-getXnemesis())*(x-getXnemesis())+(b[i]-getZnemesis())*(b[i]-getZnemesis()))<sw)
	{
	sw=100;
	distancia=1;
	if(mov2>0)	
		...
		a[i]=a[i]+0.1;
	z3=((getZnemesis()-b[i])*(a[i]-x)/(getXnemesis()-x))+b[i];
 	}
	i++;
}*/
if(aux2==1){
